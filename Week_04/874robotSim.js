...
// 思路：用 set 存储障碍坐标，每次走一步，遇到障碍则停下来
// direct: 0 top ｜ 1 right ｜ 2 down ｜ 3 left
const robotSim = function(commands, obstacles) {
  let x = 0, y = 0, direct = 0, maxDist = 0
  const obsSet = new Set(obstacles.map(pos => pos.join())),
        dx = [0, 1, 0, -1],
        dy = [1, 0, -1, 0]
  for(const command of commands) {
    if (command === -2) {
      ...
      for (let i = 0; i < command; ++i) {
        const px = x + dx[direct]
        const py = y + dy[direct]
        if (!obsSet.has(`${px},${py}`)) {
          x = px
          y = py
        } else {
