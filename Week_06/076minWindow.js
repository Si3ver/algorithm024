...
var minWindow = function(s, t) {
    ...
    let start = 0, minSize = Infinity, needCnt = t.length;
    for (let i = j = 0; j < s.length; ++j) {
        // Step1: 右边界右移，窗口增加元素
        const jChar = s[j];
        if (need.has(jChar)) {
            if (need.get(jChar) > 0) { // !! 这个 if 很关键，减到零之后，就不管了
                --needCnt;
            }
        ...
        if (needCnt === 0) {
            let iChar = s[i];
            while (!need.has(iChar) || need.get(iChar) < 0) { // 有富余，尽管移
                if (need.has(iChar)) {
...
// console.log(minWindow('DOABECODEBANC', 'ABC')); // BANC
